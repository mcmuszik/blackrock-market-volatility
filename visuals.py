...
def create_treemap() -> plotly.graph_objs._figure.Figure:
    company_info = pd.read_gbq(f'select industry, sector, ticker from {DATASET}.company_info')
    price_data = pd.read_gbq(f'select * from {DATASET}.prices')
    ...
    most_recent['volatility'] = (most_recent['long_term_ma'] - most_recent['short_term_ma'] )
